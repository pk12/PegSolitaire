...
//    Enable if you use manhattan Heuristic
    ...
    public NodeAStar(int[][] Board, int Xmax, int Ymax, Node Parent, int OldI, int OldJ, int NewI, int NewJ) {
        super(Board,Xmax,Ymax,Parent,OldI,OldJ,NewI,NewJ);
        goalx = Math.ceil((double)(Xmax / 2));
        goaly = Math.ceil((double)(Ymax / 2));
        this.children = new ArrayList<NodeAStar>();
    ...
    public void Expand() {
        ...
        while(!this.iQueue.isEmpty() && !this.jQueue.isEmpty()) {
            ...
            if (i > 1 && this.Board[i - 1][j] == 1 && this.Board[i - 2][j] == 2) {
                ...
                Clone[i - 1][j] = 2;
                Clone[i - 2][j] = 1;
                node = new NodeAStar(Clone, Xmax, Ymax, this, i, j, i - 2, j);
            ...
            if (j > 1 && this.Board[i][j - 1] == 1 && this.Board[i][j - 2] == 2) {
                ...
                Clone[i][j - 1] = 2;
                Clone[i][j - 2] = 1;
                node = new NodeAStar(Clone, Xmax, Ymax, this, i, j, i, j - 2);
                this.children.add(node);
            }

            if (j < Ymax - 2 && this.Board[i][j + 1] == 1 && this.Board[i][j + 2] == 2) {
                Clone = (int[][])Arrays.stream(this.Board).map((var0) -> {
                    return (int[])var0.clone();
                }).toArray((var0) -> {
                    return new int[var0][];
            ...
            if (i < Xmax - 2 && this.Board[i + 1][j] == 1 && this.Board[i + 2][j] == 2) {
                Clone = (int[][])Arrays.stream(this.Board).map((var0) -> {
                    return (int[])var0.clone();
                }).toArray((var0) -> {
                    return new int[var0][];
...
//        if (IsolatedPegs != 0) {
//            this.Heuristic = this.counter * IsolatedPegs;
//        } else {
//            this.Heuristic = this.counter;
//        }
    ...
    public void setgScore(int var1) {
        this.gScore = var1;
    }
